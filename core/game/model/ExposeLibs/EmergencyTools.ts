...
export function createEmergencyTools(runtime: RoomRuntime) {
    return {
    ...
    chat: function(msg: string, playerID?: number): void { // send chat
        if(playerID) {
            runtime.room.sendAnnouncement(msg, playerID, 0xFFFF00, "bold", 2);
            console.log(`[EMERGENCY.CHAT] the message is sent to #${playerID}. message: ${msg}`);
        } else {
            runtime.room.sendAnnouncement(msg, null, 0xFFFF00, "bold", 2);
