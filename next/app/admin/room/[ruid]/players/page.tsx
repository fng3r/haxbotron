...
export default function RoomPlayerList() {
    ...
    const getPlayerAccountList = async (page: number, searchQuery: string = '') => {
        ...
        try {
            const result = await client.get(`/api/v1/playerlist/${ruid}?searchQuery=${searchQuery}&start=${index}&count=${pagingCount}`);
        ...
        try {
            ...
            if (result.status === 200) {
                ...
                const onlinePlayersInfoList: Player[] = await Promise.all(onlinePlayersID.map(async (playerID) => {
                    const result: Player = await client.get(`/api/v1/room/${ruid}/player/${playerID}`)
                            ...
                            <Table size="small">
                                ...
                                <TableBody>
                                    {onlinePlayerList && onlinePlayerList.map((item, idx) => (
                                        <OnlinePlayerRow key={idx} row={item} ruid={ruid} />
                                    ))}
                                </TableBody>
                            </Table>
                        </React.Fragment>
