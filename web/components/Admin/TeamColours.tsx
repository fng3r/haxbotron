...
export default function RoomTeamColours({ ruid }: { ruid: string }) {
  const [colourPick, setColourPick] = useState('#ffffff');

  const { data: teamColoursData, error: teamColoursError } = queries.getTeamColours(ruid);
  const setTeamColoursMutation = mutations.setTeamColours();

  if (teamColoursError) {
    SnackBarNotification.error(teamColoursError.message);
  }

  const [redTeamColours, setRedTeamColours] = useState({
    angle: teamColoursData?.red.angle ?? 0,
    textColour: teamColoursData?.red.textColour ?? 0xffffff,
    teamColour1: teamColoursData?.red.teamColour1 ?? 0xe66e55,
    teamColour2: teamColoursData?.red.teamColour2 ?? 0xe66e55,
    teamColour3: teamColoursData?.red.teamColour3 ?? 0xe66e55,
  } as TeamColours);
  const [blueTeamColours, setBlueTeamColours] = useState({
    angle: teamColoursData?.blue.angle ?? 0,
    textColour: teamColoursData?.blue.textColour ?? 0xffffff,
    teamColour1: teamColoursData?.blue.teamColour1 ?? 0x5a89e5,
    teamColour2: teamColoursData?.blue.teamColour2 ?? 0x5a89e5,
    teamColour3: teamColoursData?.blue.teamColour3 ?? 0x5a89e5,
  } as TeamColours);
  ...
  const [newAngle, setNewAngle] = useState(teamColoursData?.red.angle.toString() ?? '0');
  const [newTextColour, setNewTextColour] = useState(`#${teamColoursData?.red.textColour.toString(16) ?? 'ffffff'}`);
  const [newTeamColour1, setNewTeamColour1] = useState(`#${teamColoursData?.red.teamColour1.toString(16) ?? 'ffffff'}`);
  const [newTeamColour2, setNewTeamColour2] = useState(`#${teamColoursData?.red.teamColour2.toString(16) ?? 'ffffff'}`);
  const [newTeamColour3, setNewTeamColour3] = useState(`#${teamColoursData?.red.teamColour3.toString(16) ?? 'ffffff'}`);
            ...
            <React.Fragment>
              ...
              <form className="mt-6 w-full" onSubmit={handleColoursApply} method="post">
                ...
                <Grid container spacing={2} my={1}>
                  <Grid>
                    <ChromePicker disableAlpha color={colourPick} onChange={handleColourChange} />
                  </Grid>
                  <Grid>
