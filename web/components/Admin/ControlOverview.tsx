...
import { queries } from '@/lib/queries/control';

export default function ControlOverview() {
  const { data: summary } = queries.getSummary();
  const { data: hosts } = queries.getHosts();
  const { data: rooms } = queries.getManagedRooms();
    ...
    <div className="flex flex-col gap-6">
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent className="text-3xl font-semibold">{summary?.hostCount || 0}</CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Healthy</CardTitle>
          </CardHeader>
          <CardContent className="text-3xl font-semibold">{summary?.healthyHostCount || 0}</CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Configured Rooms</CardTitle>
          </CardHeader>
          <CardContent className="text-3xl font-semibold">{summary?.configuredRoomCount || 0}</CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Integrity Issues</CardTitle>
          </CardHeader>
          <CardContent className="text-3xl font-semibold">{summary?.integrityIssueCount || 0}</CardContent>
        </Card>
      </div>
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              {rooms?.filter((room) => room.integrity === 'wrong_host').length || 0} rooms are reporting placement issues.
            </p>
          </CardContent>
        </Card>
      </div>
